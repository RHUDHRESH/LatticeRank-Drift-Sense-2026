...
COLUMNS = io_contract.COLUMNS
BUDGET_S = io_contract.BUDGET_S              # compatibility for external runners
FAILURE_SCORE = io_contract.FAILURE_SCORE
...
def _pick_search(row: dict) -> str | None:
    names = {str(name).strip().lower(): name for name in row if name}
    for key in io_contract.SEARCH_KEYS:
        source_name = names.get(key)
    ...
    for lowered_name, source_name in names.items():
        if lowered_name in FORBIDDEN_REFERENCE_KEYS:
            continue
        if any(hint in lowered_name for hint in io_contract.REF_HINTS):
            continue
        if any(hint in lowered_name for hint in io_contract.SEARCH_HINTS):
            value = row[source_name]
            if value not in (None, "") and str(value).strip().lower().endswith(io_contract.IMAGE_SUFFIXES):
                return str(value)
    return None
...
def _pick_id(row: dict, fallback: str) -> str:
    names = {str(name).strip().lower(): name for name in row if name}
    for key in io_contract.ID_KEYS:
        source_name = names.get(key)
        if source_name is not None:
            value = row[source_name]
            if value not in (None, ""):
...
def load_reference(gds_path: Path) -> np.ndarray:
    ...
    return io_contract.load_image(gds_path)
...
def process(pair_id: str, gds_path: str | None, search_path: str | None,
    ...
    try:
        ...
        resolved_gds = io_contract.resolve_path(gds_path)
        search = io_contract.load_image(io_contract.resolve_path(search_path))
        resolved_search_gds = (io_contract.resolve_path(search_gds_path)
...
def main(argv: list[str] | None = None) -> int:
    ...
    io_contract.SEARCH_ROOTS.clear()
    for root in (args.input.resolve().parent, PROJECT):
        if root not in io_contract.SEARCH_ROOTS:
            io_contract.SEARCH_ROOTS.append(root)
